...
class Cube(orb.cube.SpectralCube):
    ...
    def extract_spectrum(self, ix, iy):
        ...
        for iloop in range(self.p.depth()):
            ...
            print(idata.min(), idata.max(), np.any(np.isnan(idata)))
...
class Explorer(Cube):
    ...
    def show(self):
        ...
        def redraw_image(event):
            self.image_ax.cla()
            vmin, vmax = np.nanpercentile(df, [30,self.p.perc()])
